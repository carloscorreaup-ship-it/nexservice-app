      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 py-2 sm:py-2.5 flex items-center justify-between gap-2 sm:gap-3 relative">
        ...
        <div className="flex items-center gap-2 sm:gap-2.5 min-w-0">
          {showBackButton ? (
            <button
              onClick={onBack}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold text-xs transition-all border border-slate-200 shrink-0 active:scale-95 cursor-pointer"
            >
            ...
            >
              ...
              <div className="flex flex-col justify-center min-w-0">
                <div className="font-bold text-xs sm:text-sm tracking-tight text-[#141b2b] leading-tight font-geist flex items-center gap-1">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          <div className="relative" ref={dropdownRef}>
            <button
              ...
              className={`flex items-center gap-1 text-[11px] sm:text-xs font-medium px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full transition-all border shrink-0 cursor-pointer active:scale-95 ${
                isCityDropdownOpen
                ...
                <div className="fixed inset-x-3 top-16 md:absolute md:top-full md:right-0 md:left-auto md:w-84 mt-1.5 bg-white border border-slate-200/90 rounded-2xl shadow-elevation-hover z-50 p-3 max-h-[85vh] flex flex-col animate-in fade-in zoom-in-95 duration-150">
                  ...
                  <div className="relative mb-2">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-9 pr-8 py-2 text-xs text-[#141b2b] placeholder-slate-400 focus:outline-none focus:border-[#0052ff] focus:bg-white shadow-inner transition-all"
                    />
                    {citySearchTerm && (
                      <button
                  ...
                  <div className="overflow-y-auto max-h-56 pr-0.5 space-y-1 divide-y divide-slate-50">
                    ...
                    {filteredCities.length === 0 && (
                      <div className="p-4 text-center">
                        <p className="text-xs text-slate-500 mb-2">No se encontró "{citySearchTerm}"</p>
                        <button
          ...
          {isAdmin && (
            <button
              ...
              className={`flex items-center gap-1 p-1.5 sm:px-2.5 sm:py-1.5 rounded-full text-xs font-bold transition-all border shrink-0 cursor-pointer active:scale-95 ${
                activeTab === 'admin'
          ...
          <button
            ...
            className={`flex items-center gap-1 p-1.5 sm:px-2.5 sm:py-1.5 rounded-full text-xs font-medium border transition-all shrink-0 cursor-pointer active:scale-95 ${
              isFirebaseConnected
          ...
          <div
            ...
            className="cursor-pointer w-7 h-7 sm:w-8 sm:h-8 rounded-full ring-2 ring-[#0052ff]/20 overflow-hidden bg-slate-100 flex items-center justify-center text-[10px] sm:text-xs font-bold text-[#0052ff] hover:ring-[#0052ff] shrink-0 active:scale-95"
            title="Mi Perfil"
          >
            {userAvatarUrl ? (
        ...
        <div className="max-w-md mx-auto">
          <div className="bg-slate-200/80 p-0.5 sm:p-1 rounded-2xl flex items-center gap-1 shadow-inner">
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 px-3 rounded-xl text-xs font-bold transition-all cursor-pointer select-none active:scale-[0.98] ${
                !isProviderMode
